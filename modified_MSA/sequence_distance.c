...
{
        uint32_t dist, dist_test;
        if(len_a > len_b){
                //printf("Calc dist a\n\r");
                //dist = BPM(seq_a, seq_b, len_a, len_b);
                dist_test = bpm(seq_a, seq_b, len_a, len_b);
        }else{
                //printf("Calc dist b\n\r");
                //dist = BPM(seq_b, seq_a, len_b, len_a);
                dist_test = bpm(seq_a, seq_b, len_b, len_a);
        }
        //printf("Distance from BPM Block: %d, Distance from BPM single: %d\n\r", dist, dist_test);
        return (float)dist_test;
}
